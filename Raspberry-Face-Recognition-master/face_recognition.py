...
for frame in camera.capture_continuous(rawCapture,format="bgr",use_video_port=True):
	...
	for(x,y,w,h) in faces:
		...
		#If not exist, then it is Unknown
				...
				now = time.localtime()
				timeNow = "%02d-%02d-%02d %02d:%02d:%02d" % (now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec)
                                result = firebase.post('/chatmodel/HxYcAOS8lJYQxeWxxMXoKqV4V7m2',{'message':msgText,'timeStamp':timeNow, 'userModel':{'name':'bot', 'photo_profil':'https://cdn.imgbin.com/4/0/14/imgbin-robotics-technology-computer-icons-internet-bot-robotics-black-robot-face-illustration-RLFfkWFiZBkWMdYpMVEefSgC0.jpg'}})
				firebase.put('childNum','number',childNum)
